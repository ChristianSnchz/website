      ...
      <div className="space-y-6">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-xl font-arcade">
            {data.history.find((h) => h.key === 'santander')?.position}
          </h2>
        </div>

        <div className="text-sm text-muted-foreground">
          {data.history.find((h) => h.key === 'santander')?.startDate} -{' '}
          {data.history.find((h) => h.key === 'santander')?.endDate}
        </div>

        <div className="space-y-4 text-left">
          <p>{data.history.find((h) => h.key === 'santander')?.description}</p>

          <h3 className="font-medium mt-6">Responsabilities</h3>
          <ul className="list-disc pl-5 space-y-2">
            {data.history
              .find((h) => h.key === 'santander')
